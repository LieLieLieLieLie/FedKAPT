"""
core/prototype.py — Module 1: Federated Private Prototype Extraction.
"""

import numpy as np
from sklearn.cluster import KMeans
from sklearn.cluster import AgglomerativeClustering
from sklearn.mixture import GaussianMixture
from sklearn.preprocessing import normalize

from config import Config
from core.privacy import add_dp_noise, privacy_report


def _public_projection(features, out_dim, seed=20260811):
    """Project records with a public data-independent orthonormal map."""
    in_dim = features.shape[1]
    if out_dim is None or out_dim <= 0 or out_dim >= in_dim:
        return features.astype(np.float32), None
    rng = np.random.default_rng(seed + in_dim * 1009 + out_dim)
    basis = rng.normal(size=(in_dim, out_dim))
    basis, _ = np.linalg.qr(basis)
    return (features @ basis[:, :out_dim]).astype(np.float32), basis[:, :out_dim].astype(np.float32)


def labeled_prototypes(features, labels, n_classes, max_norm):
    # Clip every record before averaging. Under fixed public class membership
    # and replace-one adjacency, this gives sensitivity 2*C/n_k.
    norms = np.linalg.norm(features, axis=1, keepdims=True)
    clipped = features * np.minimum(1.0, max_norm / (norms + 1e-8))
    d = features.shape[1]
    P = np.zeros((n_classes, d), dtype=np.float32)
    counts = np.zeros(n_classes, dtype=np.int64)
    for k in range(n_classes):
        m = labels == k
        counts[k] = int(m.sum())
        if m.sum() > 0:
            P[k] = clipped[m].mean(axis=0)
    return P, counts


def unlabeled_prototypes(features, n_clusters, n_init=20, max_iter=500, seed=42,
                         algorithm="kmeans"):
    """Construct target prototypes with a predeclared clustering algorithm."""
    x_norm = normalize(features, norm="l2")
    algorithm = str(algorithm).lower()
    if algorithm == "kmeans":
        model = KMeans(n_clusters=n_clusters, n_init=n_init,
                       max_iter=max_iter, random_state=seed)
        assignments = model.fit_predict(x_norm)
        centers = model.cluster_centers_
    elif algorithm == "agglomerative":
        model = AgglomerativeClustering(n_clusters=n_clusters, linkage="ward")
        assignments = model.fit_predict(x_norm)
        centers = np.stack([
            x_norm[assignments == k].mean(axis=0) for k in range(n_clusters)
        ])
    elif algorithm == "gmm":
        model = GaussianMixture(
            n_components=n_clusters, covariance_type="diag",
            n_init=max(1, min(n_init, 5)), max_iter=max_iter,
            random_state=seed, reg_covar=1e-6)
        assignments = model.fit_predict(x_norm)
        centers = model.means_
    else:
        raise ValueError(
            f"Unknown clustering algorithm '{algorithm}'. "
            "Choose from: kmeans, agglomerative, gmm.")
    counts = np.bincount(assignments, minlength=n_clusters)
    return (centers.astype(np.float32), assignments.astype(np.int64),
            counts.astype(np.int64))


class PrototypeBank:
    def __init__(self, cfg: Config, side: str):
        assert side in ("t", "d")
        self.cfg  = cfg
        self.side = side
        self.rng  = np.random.default_rng(cfg.seed)

        self.raw_prototypes   = None
        self.noisy_prototypes = None
        self.assignments      = None
        self.sigma            = None
        self.release_projection = None
        self.counts = None

    def build_d_side(self, features, labels) -> "PrototypeBank":
        projected, self.release_projection = _public_projection(
            features, getattr(self.cfg.privacy, "release_dim", None))
        self.raw_prototypes, counts = labeled_prototypes(
            projected, labels, self.cfg.data.n_classes,
            self.cfg.privacy.max_norm)
        self.counts = counts
        self.noisy_prototypes, self.sigma = add_dp_noise(
            self.raw_prototypes, self.cfg.privacy.epsilon,
            self.cfg.privacy.delta, self.cfg.privacy.max_norm, self.rng,
            counts=counts)
        return self

    def build_t_side(self, features) -> "PrototypeBank":
        cfg_p = self.cfg.prototype
        centers, assignments, counts = unlabeled_prototypes(
            features, cfg_p.n_clusters, cfg_p.kmeans_n_init,
            cfg_p.kmeans_max_iter, self.cfg.seed,
            getattr(cfg_p, "clustering_algorithm", "kmeans"))
        self.raw_prototypes  = centers
        self.assignments     = assignments
        # Target-side clusters are consumed locally for semantic assignment and
        # are not transmitted as a federated message. Keep them clean; DP is
        # applied to the transmitted d-side class prototypes.
        self.noisy_prototypes = self.raw_prototypes.copy()
        self.sigma = np.zeros((len(self.raw_prototypes), 1), dtype=np.float32)
        return self

    def transmit(self) -> np.ndarray:
        return self.noisy_prototypes.copy()

    def privacy_report(self) -> dict:
        return privacy_report(
            self.cfg.privacy.epsilon, self.cfg.privacy.delta,
            self.cfg.privacy.max_norm, self.sigma)
