...
from core.ot_alignment import constrained_bijection
...
def _single_best_method(results, methods, metric):
    ...
    if len(tied) == 0:
        return None
    ours_idx = methods.index("FedKAPT (Ours)") if "FedKAPT (Ours)" in methods else -1
...
def _source_prototypes(data, cfg):
    ...
    return PrototypeBank(cfg, "d").build_d_side(
        data.d_train_x, data.d_train_y).transmit().astype(np.float32)
...
def _target_pseudo_labels(data, cfg, return_mapping: bool = False):
    ...
    if cfg.data.dataset == "office_home":
        ...
        cluster_to_class = constrained_bijection(
            -R, getattr(cfg.ot, "public_anchors", None))
    else:
...
def run_coral_ftl(data, cfg, logger=None) -> Dict:
    """CORAL covariance-alignment baseline."""
    ...
    pseudo_train, _, _ = _target_pseudo_labels(data, cfg)
    trainer = DownstreamTrainer(cfg, "CORAL-FTL")
    trainer.train(proj_tr, pseudo_train, logger)
    return trainer.evaluate(proj_te, data.t_test_y)


def run_ls_proto_align(data, cfg, logger=None) -> Dict:
    """Least-squares prototype-alignment baseline."""
    n_cls = cfg.data.n_classes
    t_dim = data.t_train_x.shape[1]
    d_dim = data.d_train_x.shape[1]
    align_dim = min(t_dim, d_dim)
    ...
    trainer = DownstreamTrainer(cfg, "LS-ProtoAlign")
    trainer.train(aligned_tr, pseudo_train, logger)
    return trainer.evaluate(aligned_te, data.t_test_y)


def run_entropy_st(data, cfg, logger=None) -> Dict:
    """Target-side entropy self-training baseline."""
    device = torch.device(cfg.device)
    pseudo_train, _, _ = _target_pseudo_labels(data, cfg)
    trainer = DownstreamTrainer(cfg, "Entropy-ST")
    trainer.train(data.t_train_x, pseudo_train, logger)
...
def run_proto_ftl(data, cfg, logger=None) -> Dict:
    """DP-prototype augmentation without OT or generation."""
    n_cls = cfg.data.n_classes
    t_dim = data.t_train_x.shape[1]
    d_dim = data.d_train_x.shape[1]
    ...
    trainer = DownstreamTrainer(cfg, "DP-ProtoAug")
    trainer.train(X_aug_tr, pseudo_train, logger)
    return trainer.evaluate(X_aug_te, data.t_test_y)


def run_ls_proto_align_fair(data, cfg, logger=None) -> Dict:
    """Least-squares prototype alignment under the same protocol."""
...
def run_entropy_st_fair(data, cfg, logger=None) -> Dict:
    """Entropy-based target self-training under the same protocol."""
    pseudo_train, _, _ = _target_pseudo_labels(data, cfg)
    trainer = DownstreamTrainer(cfg, "Entropy-ST")
    trainer.train(data.t_train_x, pseudo_train, logger)
...
def _intra_relational_cost(x):
    d = np.linalg.norm(x[:, None, :] - x[None, :, :], axis=-1)
    d /= d.max() + 1e-8
    q = np.linspace(0.0, 1.0, getattr(Config().ot, "signature_dim", 32))
    sig = np.stack([
        np.quantile(np.delete(row, i) if len(row) > 1 else row, q)
        for i, row in enumerate(d)
    ])
    return d, sig


def _transport_labels(data, cfg, mode):
    n_cls = cfg.data.n_classes
    km = KMeans(n_clusters=n_cls, n_init=cfg.prototype.kmeans_n_init,
                max_iter=cfg.prototype.kmeans_max_iter,
                random_state=cfg.seed)
    clusters = km.fit_predict(normalize(data.t_train_x, norm="l2"))
    mu_t = km.cluster_centers_.astype(np.float64)
    mu_d = _source_prototypes(data, cfg).astype(np.float64)
    Ct, st = _intra_relational_cost(mu_t)
    Cd, sd = _intra_relational_cost(mu_d)
    R = ((st[:, None, :] - sd[None, :, :]) ** 2).mean(-1)
    R /= R.max() + 1e-8
    a = np.ones(len(mu_t)) / len(mu_t)
    b = np.ones(len(mu_d)) / len(mu_d)
    if mode == "full":
        T = ot.emd(a, b, R)
    elif mode == "uot":
        T = ot.unbalanced.sinkhorn_knopp_unbalanced(
            a, b, R, reg=0.05, reg_m=1.0)
    elif mode == "gw":
        T = ot.gromov.gromov_wasserstein(
            Ct, Cd, a, b, "square_loss", log=False)
    else:
        raise ValueError(mode)
    score = T / (T.sum(axis=1, keepdims=True) + 1e-10)
    if score.shape[0] == score.shape[1]:
        mapping = constrained_bijection(
            score, getattr(cfg.ot, "public_anchors", None))
    else:
        mapping = score.argmax(axis=1)
    return mapping[clusters]


def _run_transport_baseline(data, cfg, mode, name, logger=None):
    pseudo = _transport_labels(data, cfg, mode)
    trainer = DownstreamTrainer(cfg, name)
    trainer.train(data.t_train_x, pseudo, logger)
    return trainer.evaluate(data.t_test_x, data.t_test_y)


def run_rel_full_ot(data, cfg, logger=None):
    return _run_transport_baseline(data, cfg, "full", "Rel-FullOT", logger)


def run_rel_uot(data, cfg, logger=None):
    return _run_transport_baseline(data, cfg, "uot", "Rel-UOT", logger)


def run_gw_ftl(data, cfg, logger=None):
    return _run_transport_baseline(data, cfg, "gw", "GW-FTL", logger)
...
METHODS = {
    ...
    "CORAL-FTL": run_coral_ftl,
    "LS-ProtoAlign": run_ls_proto_align_fair,
    "Entropy-ST": run_entropy_st_fair,
    "DP-ProtoAug": run_proto_ftl,
    "Rel-FullOT": run_rel_full_ot,
    "Rel-UOT": run_rel_uot,
    "GW-FTL": run_gw_ftl,
}


def run_baseline_comparison(data, cfg, fedkapt_results: Dict,
                             logger=None) -> Dict:
    all_results = {}
    for name, fn in METHODS.items():
        ...
        try:
            ...
            all_results[name] = {"accuracy": float("nan"),
                                 "macro_f1": float("nan"),
                                 "macro_auc": float("nan")}
    all_results["FedKAPT (Ours)"] = dict(fedkapt_results.get("FedKAPT", {}))
    return all_results
...
def plot_baseline_comparison(results: Dict, save_dir: str,
    ...
    fp_gi = methods.index("FedKAPT (Ours)") if "FedKAPT (Ours)" in methods else -1
    best_methods = {
