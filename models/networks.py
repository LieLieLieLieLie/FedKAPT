...
class CVAEEncoder(nn.Module):
    ...
    encoder could trivially pass cb through z and let the release-space decoder
    output cb without using any xb information — causing generated release-space
    features to be constant per cluster and freezing all Acc/F1 metrics.
...
class CVAE(nn.Module):
    def __init__(self, x_dim, cond_dim, out_dim, hidden_dims, latent_dim, dropout=0.1):
        ...
        self.cycle_encoder = MLP(
            out_dim, list(reversed(hidden_dims)), latent_dim, dropout)
    ...
    def forward(self, x, cond):
        ...
        z_cycle = self.cycle_encoder(x_mu)
        return x_mu, x_lv, z_mu, z_lv, z_cycle
