"""
trainer.py �?FedKAPT pipeline orchestrator.
...
class FedKAPTTrainer:
    def __init__(self, cfg: Config):
        self.cfg     = cfg
        self.logger  = get_logger("FedKAPT", cfg.log_dir, cfg.exp_name)
        set_seed(cfg.seed)
        os.makedirs(cfg.save_dir, exist_ok=True)
        self.data = self.t_bank = self.d_bank = None
        self.aligner = self.generator = self.filt = None
        self.baseline_trainer = self.fedkapt_trainer = None
        self.align_trainer = None
        self.combo_trainer = None
        self.target_refine_trainer = None
    ...
    def save(self):
        ...
        if self.fedkapt_trainer and self.fedkapt_trainer.model:
            torch.save(self.fedkapt_trainer.model.state_dict(),
                       os.path.join(d, "classifier_fedkapt.pt"))
        if self.aligner:
    ...
    def load(self):
        ...
        self.fedkapt_trainer = DownstreamTrainer(self.cfg, "FedKAPT")
        self.fedkapt_trainer.model = DownstreamClassifier(
            aug_dim, self.cfg.data.n_classes, cfg_ds.hidden_dims, cfg_ds.dropout).to(self.cfg.device)
        self.fedkapt_trainer.model.load_state_dict(
            torch.load(os.path.join(d, "classifier_fedkapt.pt"), map_location=self.cfg.device))
    ...
    def phase2_ot_alignment(self):
        ...
        # The source-to-target information contract permits only the released
        # DP prototypes. Alignment on raw source prototypes would not be
        # post-processing of the declared Gaussian mechanism.
        if getattr(self.cfg.ot, "clean_alignment", False):
            raise ValueError(
                "clean_alignment=True violates the one-message DP contract; "
                "alignment must consume the sanitized prototype release.")
        proto_d_for_ot = self.d_bank.transmit()
    ...
    def phase3_generate(self):
        ...
        if getattr(self.cfg.cvae, "disable_generation", False):
            self.generator = None
            gen_train = cond_train.copy()
            gen_test = cond_test.copy()
            var_train = np.zeros(len(gen_train), dtype=np.float32)
            cycle_mse = float("nan")
        else:
            self.generator = CVAEGenerator(self.cfg)
            self.generator.train(self.data.t_train_x, cond_train, self.logger)
            gen_train, var_train = self.generator.generate(
                self.data.t_train_x, cond_train)
            gen_test, _ = self.generator.generate(self.data.t_test_x, cond_test)
            cycle_mse = self.generator.latent_cycle_mse(
                self.data.t_train_x, cond_train)
        # Post-generation smoothing: blend CVAE output with the (smooth)
        # OT transport condition.  This reduces CVAE generation noise and
        # can improve probability ranking (AUC) while retaining OT alignment.
        ...
        self.generation_diagnostics = self._generation_diagnostics(
            self.data.t_train_x, gen_train, cond_train, pseudo_labels,
            cycle_mse)
        self.timer.record("Phase3: CVAE Generation", time.time()-t0)
        return gen_train, var_train, cond_train, pseudo_labels, gen_test

    @staticmethod
    def _cosine_dispersion(features, labels):
        ...
        for label in np.unique(labels):
            ...
            values.extend((1.0 - block @ center).tolist())
            centers.append(center)
        within = float(np.mean(values)) if values else float("nan")
        if len(centers) > 1:
            c = np.stack(centers)
            dist = 1.0 - c @ c.T
            between = float(dist[np.triu_indices(len(c), 1)].mean())
        else:
            between = float("nan")
        return within, between

    def _generation_diagnostics(self, target, generated, conditions, labels,
                                cycle_mse):
        gen_within, gen_between = self._cosine_dispersion(generated, labels)
        tgt_within, _ = self._cosine_dispersion(target, labels)
        g = np.asarray(generated, dtype=np.float64)
        c = np.asarray(conditions, dtype=np.float64)
        condition_rmse = float(np.sqrt(np.mean((g - c) ** 2)))
        return {
            "generated_within_class_cosine_dispersion": gen_within,
            "generated_interclass_cosine_distance": gen_between,
            "target_within_class_cosine_dispersion": tgt_within,
            "sample_specificity_ratio": float(gen_within / (tgt_within + 1e-12)),
            "condition_rmse": condition_rmse,
            "latent_cycle_mse": float(cycle_mse),
        }
    ...
    def phase5_train_classifiers(self, gen_train, gen_test, pseudo_labels, sample_weight):
        ...
        if getattr(self.cfg.downstream, "use_confidence_refinement", False):
            logits_t = self.baseline_trainer.predict_logits(self.data.t_train_x)
            probs_t = np.exp(logits_t - logits_t.max(axis=1, keepdims=True))
            probs_t /= probs_t.sum(axis=1, keepdims=True) + 1e-12
            confidence_t = probs_t.max(axis=1)
            quantile_t = float(getattr(
                self.cfg.downstream, "confidence_refinement_quantile", 0.60))
            keep_t = confidence_t >= np.quantile(confidence_t, quantile_t)
            labels_t = probs_t.argmax(axis=1)
            if (keep_t.sum() >= self.cfg.data.n_classes
                    and np.unique(labels_t[keep_t]).size
                    == self.cfg.data.n_classes):
                self.target_refine_trainer = DownstreamTrainer(
                    self.cfg, "FedKAPT-target-refine")
                self.target_refine_trainer.train(
                    self.data.t_train_x[keep_t], labels_t[keep_t], self.logger)
        X_aug_tr = np.concatenate([self.data.t_train_x, gen_train], axis=1)
        self.fedkapt_trainer = DownstreamTrainer(self.cfg, "FedKAPT")
        self.fedkapt_trainer.train(X_aug_tr, pseudo_labels, self.logger,
                                  sample_weight=sample_weight)
        if getattr(self.cfg.downstream, "use_confidence_refinement", False):
            logits = self.fedkapt_trainer.predict_logits(X_aug_tr)
            ...
            refined_labels = probs.argmax(axis=1)
            if (keep.sum() >= self.cfg.data.n_classes
                    and np.unique(refined_labels[keep]).size
                    == self.cfg.data.n_classes):
                self.fedkapt_trainer.train(
                    X_aug_tr[keep], refined_labels[keep], self.logger)
                self.logger.info(
                    "  [FedKAPT] confidence refinement retained "
                    f"{int(keep.sum())}/{len(keep)} target records")
        align_tr, self._align_W = self._prototype_alignment_view(self.data.t_train_x, fit=True)
        self.align_trainer = DownstreamTrainer(self.cfg, "FedKAPT-align")
        ...
        # down-weight generated release-space features; applying them here weakens the align
        ...
        if getattr(self.cfg.downstream, "use_combo_fusion", False):
            ...
            self.combo_trainer = DownstreamTrainer(self.cfg, "FedKAPT-combo")
    ...
    def phase5_evaluate(self) -> Dict:
        ...
        if self.target_refine_trainer is not None:
            target_logits = self.target_refine_trainer.predict_logits(
                self.data.t_test_x)
        X_aug_te = np.concatenate([self.data.t_test_x, self._gen_test], axis=1)
        fed_logits = self.fedkapt_trainer.predict_logits(X_aug_te)
        ...
        if self._gen_train is not None:
            ...
            if self.target_refine_trainer is not None:
                train_base_logits = self.target_refine_trainer.predict_logits(
                    self.data.t_train_x)
            train_fed_logits = self.fedkapt_trainer.predict_logits(train_aug)
        ...
        results  = {"Baseline": res_base, "FedKAPT": res_fed}
    ...
    def _pseudo_score(self, logits, labels):
        ...
        try:
            ...
            if self.cfg.data.n_classes == 2:
                ...
                yb = label_binarize(
                    labels, classes=list(range(self.cfg.data.n_classes)))
                auc = roc_auc_score(
    ...
    def _auc_from_logits(self, logits, y_true):
        try:
            ...
            yb = label_binarize(
                y_true, classes=list(range(self.cfg.data.n_classes)))
    ...
    def _select_fusion_alpha(self, base_logits, fed_logits, labels):
        ...
        for alpha in grid:
            ...
            acc = accuracy_score(labels, pred)
            f1  = f1_score(labels, pred, average="macro", zero_division=0)
            # Include AUC so that the training-only selection criterion covers
            # probability ranking as well as hard pseudo-label agreement.  The
            # balanced score is fixed independently of target-test outcomes.
            try:
                probs = np.exp(logits - logits.max(1, keepdims=True))
                ...
                if n_cls == 2:
                    ...
                    yb = label_binarize(
                        labels, classes=list(range(n_cls)))
                    auc = roc_auc_score(
                        yb, probs, average="macro", multi_class="ovr")
            except Exception:
                auc = (acc + f1) / 2
    ...
    def _select_align_alpha(self, fused_logits, align_logits, labels):
        ...
        for alpha in grid:
            ...
            if is_oc:
                ...
                # the alignment baseline), discarding the CVAE ranking signal.
    ...
    def _header(self, mode):
        self.logger.info("="*58)
        self.logger.info(f"FedKAPT . {self.cfg.exp_name}  [{mode}]")
    ...
    def retrain_with_epsilon(self, epsilon: float) -> Dict:
        ...
        # A calibration failure must stop the sweep.  Silently falling back to
        # clean prototypes would label a non-private release as private.
        ...
        try:
            # Phase 2 must use the released noisy prototypes; using raw source
            # prototypes here would violate the stated one-message protocol.
            aligner = PartialOTAligner(self.cfg).fit(
                self.t_bank.raw_prototypes, self.d_bank.transmit())
            ...
            X_aug_tr = np.concatenate([self.data.t_train_x, gen_train], axis=1)
            fedkapt_tr = DownstreamTrainer(self.cfg, "FedKAPT")
            fedkapt_tr.train(X_aug_tr, pseudo_labels, self.logger, sample_weight=sw)

            align_tr_obj = DownstreamTrainer(self.cfg, "FedKAPT-align")
            ...
            fed_logits  = fedkapt_tr.predict_logits(X_aug_te)
            train_base_logits = baseline_tr.predict_logits(self.data.t_train_x)
            train_fed_logits = fedkapt_tr.predict_logits(X_aug_tr)
